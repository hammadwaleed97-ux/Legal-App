...
# 3. نظام إدارة الصفحات (الأيقونات الثابتة)
if 'page' not in st.session_state:
    st.session_state.page = "الرئيسية"

# توزيع الأيقونات في شبكة احترافية
c1, c2 = st.columns(2)
c3, c4 = st.columns(2)
c5, c6 = st.columns(2)
...
# 4. تفاصيل الأقسام (المعطيات الشاملة)
...
if st.session_state.page == "قضايا":
    ...
    if court_type == "القضاء العادي":
        ...
        action = st.selectbox("الإجراء:", [
            ...
            "صياغة صحيفة طعن بالنقض",
            "مذكرة دفاع (الهيئة طاعنة/مستأنفة)", 
            "مذكرة دفاع (الهيئة مطعون ضدها/مستأنف ضدها)"
        ])
    else: # مجلس الدولة
