...
# 1. إعدادات الصفحة والهوية البصرية (اللوجو الرسمي المختصر)
...
# 2. الهيكل الرئيسي للمنظومة (4 أقسام كبرى)
...
# --- القسم الثاني: الإدارة العامة للفتوى ---
with tab2:
    f_cat = st.selectbox("التصنيف:", ["فتاوى قانونية", "إصابات عمل", "شكاوى الزواج العرفى", "أرشيف الفتاوى"])
    if f_cat != "أرشيف الفتاوى":
        st.text_area("ملخص الوقائع ومثار البحث القانوني")
        st.file_uploader("إرفاق مذكرة الإحالة والمستندات")
        if st.button("📝 صياغة الرأي القانوني"):
            st.write("عضو الإدارة القانونية: ............ &nbsp;&nbsp; مدير الإدارة القانونية: ............")

# --- القسم الثالث: الإدارة العامة للتحقيقات ---
with tab3:
    inv_type = st.radio("جهة التحقيق:", ["تحقيقات الهيئة الداخلى", "النيابة الإدارية", "النيابة العامة", "أرشيف التحقيقات"], horizontal=True)
    if "أرشيف" not in inv_type:
        st.text_input("رقم التحقيق / السنة / تاريخ الإحالة")
        st.text_input("اسم المخالف (المخالفين) / المنطقة التأمينية")
        st.text_area("ملخص الوقائع والمخالفة المنسوبة")
        col_inv1, col_inv2 = st.columns(2)
        with col_inv1: st.button("📜 فتح محضر (س وج)")
        with col_inv2: st.button("📝 صياغة مذكرة التصرف")
        st.write("عضو الإدارة القانونية: ............ &nbsp;&nbsp; مدير الإدارة القانونية: ............")

# --- القسم الرابع: المكتبة القانونية (الـ 14 قسماً بالترتيب المعتمد) ---
with tab4:
    st.subheader("المكتبة الرقمية (المرجع الفني للصياغة)")
    lib_sections = [
    ...
    selected_lib = st.selectbox("اختر قسم المادة العلمية:", lib_sections)
    st.file_uploader(f"رفع مستندات جديدة إلى {selected_lib}")
